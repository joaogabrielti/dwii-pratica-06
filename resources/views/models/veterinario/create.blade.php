...
<form class="row g-2" action="@isset($veterinario){{ route('veterinario.update', ['veterinario' => $veterinario['id']]) }}@else{{ route('veterinario.store') }}@endisset" method="POST" autocomplete="off">
    ...
    <div class="col-12">
        ...
        <input class="form-control" type="nome" id="nome" name="nome" value="{{ $veterinario['nome'] ?? old('nome') }}" required autofocus>
    </div>
    <div class="col-12 col-md-6">
        <label class="form-label mb-0" for="crmv">CRMV</label>
        <input class="form-control" type="crmv" id="crmv" name="crmv" value="{{ $veterinario['crmv'] ?? old('crmv') }}" required>
    </div>
    <div class="col-12 col-md-6">
