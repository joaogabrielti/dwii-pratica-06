...
<form class="row g-2" action="@isset($especialidade){{ route('especialidade.update', ['especialidade' => $especialidade['id']]) }}@else{{ route('especialidade.store') }}@endisset" method="POST" autocomplete="off">
    ...
    <div class="col-12">
        ...
        <input class="form-control" type="nome" id="nome" name="nome" value="{{ $especialidade['nome'] ?? old('nome') }}" required autofocus>
    </div>
    <div class="col-12">
        <label class="form-label mb-0" for="descricao">Descrição</label>
        <textarea class="form-control" name="descricao" id="descricao" rows="3" required>{{ $especialidade['descricao'] ?? old('descricao') }}</textarea>
    </div>
