...
<form class="row g-2" action="@isset($cliente){{ route('cliente.update', ['cliente' => $cliente['id']]) }}@else{{ route('cliente.store') }}@endisset" method="POST" autocomplete="off">
    ...
    <div class="col-12">
        ...
        <input class="form-control" type="nome" id="nome" name="nome" value="{{ $cliente['nome'] ?? old('nome') }}" required autofocus>
    </div>
    <div class="col-12 col-md-8">
        <label class="form-label mb-0" for="email">Endereço de Email</label>
        <input class="form-control" type="email" id="email" name="email" value="{{ $cliente['email'] ?? old('email') }}" required>
    </div>
    <div class="col-12 col-md-4">
        <label class="form-label mb-0" for="telefone">Nº de Telefone</label>
        <input class="form-control" type="telefone" id="telefone" name="telefone" value="{{ $cliente['telefone'] ?? old('telefone') }}" required>
    </div>
